# Singlepagecode Module Variant

This folder contains a duplicated single-file viewer build refactored to use one inline ES module script:

- `singlepagecode-module/index.html`

## What this file is

`index.html` in this folder is a bundled viewer artifact.

It keeps the same one-file delivery model as `singlepagecode/`, but the inline JavaScript now runs inside a single `<script type="module">` block instead of many classic script blocks sharing `window.HDFViewer`.
...
`singlepagecode-module/index.html` is the same standalone flow, but with the inline JavaScript consolidated into one ES module entry.

## Important note

Treat `singlepagecode-module/index.html` as a generated or packaged output.
